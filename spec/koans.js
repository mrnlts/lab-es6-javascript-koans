...
describe('`const` is like `let` plus read-only. ', () => {
  ...
  describe('scalar values are read-only', () => {
    it('number are read-only', () => {
      const constNum = 0;
      // constNum = 1;  >> Cannot change value of a constant variable
  ...
  describe('destruct parameters', () => {
    it('multiple params from object', () => {
      const fn = (nam, id) => {
        ...
        expect(nam).toEqual('Wolfram');
      };
      const user = { nam: 'Wolfram', id: 42 };
      fn(user.nam, user.id);
    });

    it('multiple params from array/object', () => {
      const fn = ([, {nam}]) => {
        expect(nam).toEqual('Alice');
      };
      const users = [{ nam: 'nobody' }, { nam: 'Alice', id: 42 }];
      fn(users);
    });
  });
